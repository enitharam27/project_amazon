...
class OrderPage:
    ...
    def is_data_already_stored(self):
        
        sql = "SELECT COUNT(*) FROM orders WHERE customer_name = %s AND customer_id = %s AND customer_phone = %s AND customer_address = %s AND product_name = %s AND total_amount = %s"
        values = (
            self.app.customer_info_page.name_entry.get(),
            self.app.customer_info_page.id_entry.get(),
            int(self.app.customer_info_page.phone_entry.get()),  
            self.app.customer_info_page.address_entry.get(),
            self.customer_product.name,
            self.shopping_cart.calculate_total()
        )
    ...
    def place_order(self):
        ...
        if total_amount > 0 and self.customer_product:
            ...
            if not self.is_data_already_stored():
    ...
    def save_to_database(self, message):
           sql = "INSERT INTO orders (customer_name, customer_id, customer_phone, customer_address, product_name, total_amount) VALUES (%s, %s, %s, %s, %s, %s)"
           values = (
                    self.app.customer_info_page.name_entry.get(),
                    self.app.customer_info_page.id_entry.get(),
                    int(self.app.customer_info_page.phone_entry.get()), 
    ...
    def save_to_text_file(self, message, topic="----------BILL INVOICE-----------"):
        ...
        with open(r"C:\Users\vignesh\Desktop\amazon.txt", "a") as file:
            file.write(f"{topic}:\n")
            file.write(message + "\n")
            print("Data successfully stored in the text file.")
